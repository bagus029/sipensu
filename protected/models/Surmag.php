...
class Surmag extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('KETERANGANSURMAG,TGLCETAKSURATSURMAG, ACCSURMAG, PREVIEW, ACCSUBKOR, TANGGALACCSUBKOR, ACCDEKANAT, TANGGALACCDEKANAT, TANGGALACCSURMAG', 'safe'),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('IDSURMAG, KOTAINSTANSISURMAG, IDJENISSURAT,ACCSURMAG,TGLCETAKSURATSURMAG, TGLMULAISURMAG, TGLAKHIRSURMAG NOSURMAG, INSTANSISURMAG, ALAMATINSTANSISURMAG, IDTASEMESTER, KETERANGANSURMAG, TGLSUBMITSURMAG, PREVIEW', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function getListsendsurmag()
    {
        ...
        foreach ($this->detailsurmags as $nosuratmag) {
            echo CHtml::link('<h4>Permohonan Magang di Instansi&nbsp;: ' . $nosuratmag->iDSURMAG->INSTANSISURMAG . '&nbsp;<i class="fa fa-hand-o-right"></i> Telah Terkirim ke :&nbsp' . $nosuratmag->iDGROUPS->DIVISI) . '</li></h4>';
        }
        return $hasil;
    }
    ...
    public function getListbuatnosurmag()
    {
        ...
        if (count($this->detailsurmags) > 0) echo '<ul style="padding-left:15px">';
        foreach ($this->detailsurmags as $nosuratmag) {
            echo CHtml::link('<li>No.Surat : ' . $nosuratmag->iDSURMAG->NOSURMAG . '&nbsp;<i class="fa fa-hand-o-right"></i> Send :&nbsp' . $nosuratmag->iDGROUPS->DIVISI) . '</li>';
    ...
    public function getListtambahmahasiswasurmag()
    {
        ...
        foreach ($this->groupsurmags as $groupsurmag) {
            echo CHtml::link('<li>' . $groupsurmag->nIM->NAMA . '<i class="fa fa-caret-right"></i>' . $groupsurmag->NOURUTGROUPSURMAG, array('groupsurmag/update', 'id' => $groupsurmag->IDGROUPSURMAG)) . '</li>';
        }
    ...
    public function getMahasiswasurmag()
    {
        ...
        if (count($this->groupsurmags) > 0) echo '<ul style="padding-left:15px">';
        foreach ($this->groupsurmags as $groupsurmag) {
            echo CHtml::link('<li>' . $groupsurmag->nIM->NAMA . '<i class="fa fa-caret-right"></i>' . $groupsurmag->NOURUTGROUPSURMAG) . '</li>';
        }
        if (count($this->groupsurmags) > 0) echo '</ul>';
        return $hasil;
    }
    ...
    public function getViewkirimsurat()
    {
        ...
        if ($b > 0) {
            echo CHtml::link('<i class="fa fa-check"></i> View', array('surmag/sendisisurat', 'id' => $this->primaryKey), array('class' => 'btn btn-sm purple'));
        } else {
            echo CHtml::link('<i class="fa fa-tag"></i> View', array('surmag/isisurat', 'id' => $this->primaryKey), array('class' => 'btn btn-sm purple'));
        }
    }
    ...
    public function getCetakbytgl()
    {
        ...
        if ($b > 0) {
            echo CHtml::link('<i data-feather="printer"></i> QR', array('cetak/surmag', 'id' => $this->IDSURMAG, 'tipe'=>'1'), array('target' => '_blank', 'class' => 'btn btn-gradient-primary'));
            echo CHtml::link('<i data-feather="printer"></i> QR', array('cetak/surmag', 'id' => $this->IDSURMAG, 'tipe'=>'2'), array('target' => '_blank', 'class' => 'btn btn-gradient-danger'));
        } else {
