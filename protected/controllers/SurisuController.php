...
class SurisuController extends Controller
{
	/**
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','createbymahasiswa','sendisisurat','updatebymahasiswa','permintaansurat','isisurat'),
				'users'=>array('*'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','view','admin','delete','createbymahasiswa','sendisisurat','delete',
                    'updatebymahasiswa','deletebyowner','updatenosurmag','updatenosurisuoperator','permintaansurat','isisurat','AjaxTtd','notifikasi','preview'
	 ...
	 */
        ...
        $sql = "select count(IDISU) as newrecord from surisu where PREVIEW='N' ";
    ...
    public function actionPreview() {
        ...
        $sql="select * from surisu WHERE IDGROUPS='$user'";
        $data = Yii::app()->db->createCommand($sql)->queryAll();
        ...
        $this->renderPartial('preview', array('data' => $data,));
         ...
         public function actionAjaxTtd(){
                        ...
                        $model = Surisu::model()->findByPk($idisu);
                        $model->TANGGALACCSURISU = date('Y-m-d H:i:s');
                        $model->ACCSURISU = $acc;
                        if($model->update()){
                        $data['ACCSURISU']=$model->ACCSURISU;
                        }
                        else{
                        $model = Surisu::model()->findByPk($idisu);
                        $data['ACCSURISU']=$model->ACCSURISU;
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Surisu']))
		{
			$model->attributes=$_POST['Surisu'];
			if($model->save())
				$this->redirect(array('view','id'=>$model->IDISU));
		}
        ...
        public function actionCreatebymahasiswa()
	{
		...
		if(isset($_POST['Surisu']))
		{
			$model->attributes=$_POST['Surisu'];
			if($model->save())
				$this->redirect(array('isisurat','id'=>$model->IDISU));
		}
        ...
        public function actionUpdatebymahasiswa($id)
	{
		...
		$this->render('updatebymhs',array(
         ...
         public function actionUpdatenosurisu($id)
	{
		$model=$this->loadModel($id);
		...
		if(isset($_POST['Surisu']))
		{
			$model->attributes=$_POST['Surisu'];
			if($model->save())
				$this->redirect(array('admin','id'=>$model->IDISU));
		}

		$this->render('updatenosurisu',array(
			'model'=>$model,
		));
	}
        public function actionUpdatenosurisuoperator($id)
	{
		$model=$this->loadModel($id);
		...
		if(isset($_POST['Surisu']))
		{
			$model->attributes=$_POST['Surisu'];
			if($model->save())
				$this->redirect(array('detailsurisu/admin','id'=>$model->IDISU));
		}
         ...
         public function actionDeletebyowner($id)
	{
		...
		$this->redirect(array('surisu/admin'));
	}
	...
	{
		$dataProvider=new CActiveDataProvider('Surisu');
	...
	{
		$model=new Surisu('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Surisu']))
			$model->attributes=$_GET['Surisu'];
	 ...
	 * @return Surisu the loaded model
	 ...
	 * @param Surisu $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='surisu-form')
		{
